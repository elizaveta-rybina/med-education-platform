    ...
    <div className="pl-8 sm:pl-11 pb-3 sm:pb-4">
      ...
      <div className="mb-4 sm:mb-6">
        ...
        <p className="text-xs sm:text-sm text-gray-500">
          В модуле рассматривается {totalTopics} {getTopicWord(totalTopics)}
        </p>
      </div>
        ...
        <div className="mb-4 sm:mb-6">
          <div className="flex items-center justify-between mb-1">
            ...
            <span className="text-xs sm:text-sm text-gray-500">
              {completedTopics}/{totalTopics} завершено
            </span>
          </div>
      ...
      <div className="space-y-2 sm:space-y-3 mb-3 sm:mb-4">
        {topics.map((topic, index) => (
          <div key={index} className="flex items-start">
            ...
            <div>
              <p className={`text-sm sm:text-base text-gray-800 ${topic.completed ? 'opacity-75' : ''}`}>
                {topic.title}
              </p>
              <p className="text-xs sm:text-sm text-gray-500">{topic.duration}</p>
            </div>
          </div>
        ))}
      </div>
            ...
            <div>
              ...
              <p className="text-xs sm:text-sm text-gray-500">
                {completedTopics === totalTopics
                  ...
                  : `Доступен после завершения всех ${totalTopics} тем`}
              </p>
            </div>
          </div>
        </div>
...
function getTopicWord(count: number): string {
  ...
  if (lastTwoDigits >= 11 && lastTwoDigits <= 19) return 'тем';
  if (lastDigit === 1) return 'тема';
  if (lastDigit >= 2 && lastDigit <= 4) return 'темы';
  return 'тем';
}
